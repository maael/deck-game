...
function LootableContainer.new (world, x, y, tile_set_img, tile_set_grid, spritesLayer)
  ...
  lootable_container.physics.body = love.physics.newBody(world, lootable_container.x + (GRID_SIZE / 2), lootable_container.y + (GRID_SIZE / 2), "static")
  lootable_container.physics.shape = love.physics.newRectangleShape(0, 0, lootable_container.size,lootable_container.size)
  lootable_container.physics.fixture = love.physics.newFixture(lootable_container.physics.body, lootable_container.physics.shape)
...
function LootableContainer:handleLoot (player)
  if not self.is_looted then
    self.is_looted = true
    table.insert(self.spritesLayer, InteractiveEntity.new(self.world, self.x, self.y + GRID_SIZE, "pickup_item", "heal_potion", self.tile_set_img, self.tile_set_grid))
  end
end
